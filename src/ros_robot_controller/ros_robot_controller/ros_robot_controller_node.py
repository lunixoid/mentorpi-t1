...
from ros_robot_controller.t1_runtime import T1MotorGate
...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        ...
        # T1: stock MentorPi send_machine_type before any speed. Overlay image SDK
        # omitted this; topic zeros then left a latched right track.
        import ros_robot_controller.ros_robot_controller_sdk as rrc_sdk

        machine_type = os.environ.get("MACHINE_TYPE", "MentorPi_Tank")
        self.get_logger().info(
            "T1 motor-init begin sdk=%s MACHINE_TYPE=%s type=0x01x2 battery=0x1af4 zero IDs 1-4"
            % (rrc_sdk.__file__, machine_type)
        )
        self.board.apply_t1_motor_init(zero_repeats=1)
        self.motor_gate = T1MotorGate()
        self.get_logger().info(
            "T1 motor-init done (JGB37 x2, battery 0x1af4, full zero); "
            "startup_preflight=armed had_nonzero=False UART silent until first nonzero"
        )
        self._logged_runtime_zeros = 0
        ...
        self._idle_stats_timer = self.create_timer(2.0, self._log_idle_uart_stats)
        self.get_logger().info("\033[1;32m%s\033[0m" % "start")

    def _log_idle_uart_stats(self):
        self._idle_stats_timer.cancel()
        self.get_logger().info(
            "T1 motor-gate uart_runtime_speed_zero=%d halt=%d suppressed_zero=%d "
            "startup_preflight=%s had_nonzero=%s preflight_count=%d"
            % (
                getattr(self.board, "uart_runtime_speed_zeros", 0),
                getattr(self.board, "uart_halts", 0),
                self.motor_gate.zero_suppressed_count,
                "armed" if self.motor_gate.startup_preflight else "consumed",
                self.motor_gate.had_nonzero,
                self.motor_gate.preflight_count,
            )
        )

    def shutdown(self, signum, frame):
        self.get_logger().info("\033[1;32m%s\033[0m" % "shutdown")
        self.board.dispatch_motors(halt=True, speeds=None)
        self.get_logger().info(
            "T1 shutdown halt uart_runtime_speed_zero=%d halt=%d"
            % (
                getattr(self.board, "uart_runtime_speed_zeros", 0),
                getattr(self.board, "uart_halts", 0),
            )
    ...
    def set_motor_state(self, msg):
        ...
        decision = self.motor_gate.observe(data)
        if decision.startup_preflight:
            self.get_logger().info(
                "T1 startup preflight (JGB37 x2, battery 0x1af4, full zero) "
                "then original speed; startup_preflight=consumed"
            )
        elif decision.idle_preflight:
            self.get_logger().info("T1 idle preflight (JGB37 x2, battery 0x1af4, full zero) " "then original speed")
        elif decision.halt:
            self.get_logger().info("T1 runtime halt (JGB37 x2, battery 0x1af4, full zero IDs 1-4)")
        elif not decision.send_speed:
            if self.motor_gate.zero_suppressed_count == 1:
                self.get_logger().info("T1 motor-gate suppress full-zero UART until nonzero")
        self.board.dispatch_motors(halt=decision.halt, speeds=data if decision.send_speed else None)
        zeros = getattr(self.board, "uart_runtime_speed_zeros", 0)
        if zeros and zeros != self._logged_runtime_zeros:
            self._logged_runtime_zeros = zeros
            self.get_logger().warn("T1 UART runtime speed-zero count=%d" % zeros)

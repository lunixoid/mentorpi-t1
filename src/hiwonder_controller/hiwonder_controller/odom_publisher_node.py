...
import rclpy
import tf2_ros
from geometry_msgs.msg import Pose, Pose2D, PoseWithCovarianceStamped, TransformStamped, Twist
from nav_msgs.msg import Odometry
from rclpy.node import Node
from ros_robot_controller_msgs.msg import MotorsState, SetBusServoState
from std_srvs.srv import Trigger

from hiwonder_controller import ackermann, mecanum
...
class Controller(Node):
    ...
    def __init__(self, name):
        ...
        # T1 overlay: stock MentorPi_Tank geometry (not JetRover mecanum 0.216/0.195/0.097).
        self.ackermann = ackermann.AckermannChassis(wheelbase=0.1368, track_width=0.1446, wheel_diameter=0.075)
        ...
        self.declare_parameter("machine_type", os.environ.get("MACHINE_TYPE", "MentorPi_Tank"))
        ...
        self.get_logger().info(
            "T1 tank odom mapping=%s topic=%s tf=%s->%s publish_tf=%s"
            % (
                self.machine_type,
                self.odom_topic if self.pub_odom_topic else "(disabled)",
                self.odom_frame_id,
                self.base_frame_id,
                self.publish_tf,
            )
        ...
        # Sole Twist path from our adapter. Do not subscribe to /cmd_vel.
        self.create_subscription(Twist, "hiwonder_controller/cmd_vel", self.cmd_vel_callback, 1)
        self.create_service(Trigger, "hiwonder_controller/load_calibrate_param", self.load_calibrate_param)
    ...
    def cmd_vel_callback(self, msg):
        # MentorPi T1 tank: no strafe, stock wheel signs via overlay mecanum.
    ...
    def cal_odom_fun(self):
        while True:
            ...
            self.x += (
                math.cos(self.pose_yaw) * self.linear_x * self.dt - math.sin(self.pose_yaw) * self.linear_y * self.dt
            )
            self.y += (
                math.sin(self.pose_yaw) * self.linear_x * self.dt + math.cos(self.pose_yaw) * self.linear_y * self.dt
            )
            ...
            if self.odom_broadcaster is not None:

...
const char* demo_contour_text(viewer::DemoContour demo) {
  return demo == viewer::DemoContour::Active ? "active" : "inactive";
}

const char* bridge_text(viewer::Bridge bridge) {
  return bridge == viewer::Bridge::Active ? "active" : "inactive";
}

std::string websocket_url(const viewer::Status& status) {
  std::ostringstream out;
  out << "ws://" << status.host << ':' << status.port;
  return out.str();
}

void print_viewer_kv(std::ostream& out, bool color, const viewer::Status& status) {
  auto key = [&](const char* name) { paint(out, color, kDim, pad_right(name, 18)); };
  key("demo contour");
  if (status.demo == viewer::DemoContour::Active) {
    paint(out, color, kGreen, demo_contour_text(status.demo));
  } else {
    out << demo_contour_text(status.demo);
  }
  out << '\n';
  key("viewer bridge");
  if (status.bridge == viewer::Bridge::Active && status.port_listening) {
    paint(out, color, kGreen, bridge_text(status.bridge));
  } else {
    out << bridge_text(status.bridge);
  }
  out << '\n';
  key("websocket");
  out << websocket_url(status) << '\n';
  if (!status.ros_domain_id.empty()) {
    key("ros domain id");
    out << status.ros_domain_id << '\n';
  }
  key("fixed frame");
  out << viewer::kFixedFrame << '\n';
  key("robot model");
  out << viewer::kRobotDescriptionTopic << '\n';
  key("odometry");
  out << viewer::kOdomTopic << '\n';
  key("tf");
  out << viewer::kModelTfChain << '\n';
  key("sensor frames");
  out << viewer::kModelSensorFrames << '\n';
  key("lidar scan");
  out << viewer::kLidarScanTopic << '\n';
  key("lidar tf");
  out << viewer::kLidarTfFrames << '\n';
  key("camera 2d");
  out << viewer::kCamera2dHint << '\n';
  key("persons");
  out << viewer::kPersonsTopic << '\n';
  key("nearest person");
  out << viewer::kNearestPersonTopic << '\n';
  key("detections 2d");
  out << viewer::kDetections2dTopic << '\n';
  key("persons overlay");
  out << viewer::kPersonsOverlayHint << '\n';
  key("camera 3d");
  out << viewer::kCamera3dHint << '\n';
  key("camera tf");
  out << viewer::kCameraTfFrames << '\n';
  key("imu");
  out << viewer::kImuTopic << '\n';
  key("imu odom");
  out << viewer::kImuOdomTopic << '\n';
  key("imu tf");
  out << viewer::kImuTfFrames << '\n';
}

void print_viewer_connect_hint(std::ostream& out, bool color, const viewer::Status& status) {
  out << '\n';
  paint(out, color, kBold, "FOXGLOVE DESKTOP (Mac)");
  out << '\n';
  out << "  Open connection -> Foxglove WebSocket -> " << websocket_url(status) << '\n';
  if (!status.ros_domain_id.empty()) {
    out << "  ROS_DOMAIN_ID on robot: " << status.ros_domain_id << '\n';
  }
  out << "  Read-only: no command topics or services from Foxglove\n";
  out << "  3D: Fixed frame " << viewer::kFixedFrame << "; enable TF " << viewer::kModelTfChain
      << '\n';
  out << "  3D: Add RobotModel topic " << viewer::kRobotDescriptionTopic << '\n';
  out << "  3D: Add LaserScan topic " << viewer::kLidarScanTopic << " (frame "
      << viewer::kLidarFrame << ", TF " << viewer::kLidarTfFrames << ")\n";
  out << "  Image: " << viewer::kCameraColorCompressedTopic << " if listed, else "
      << viewer::kCameraColorTopic << " (color 2D)\n";
  out << "  Plot/Raw: " << viewer::kPersonsTopic << '\n';
  out << "  Plot/Raw: " << viewer::kNearestPersonTopic << '\n';
  out << "  Plot/Raw: " << viewer::kDetections2dTopic << '\n';
  out << "  Image: " << viewer::kPersonsOverlayTopic
      << " after t1ctl debug on (bgr8; same Image panel)\n";
  out << "  3D: Add PointCloud2 topic " << viewer::kCameraCloudTopic << " (frame "
      << viewer::kCameraFrame << ", TF " << viewer::kCameraTfFrames << ")\n";
  out << "  Image: depth " << viewer::kCameraDepthTopic << " if no PointCloud2 (frame "
      << viewer::kCameraFrame << ")\n";
  out << "  Plot or IMU: topic " << viewer::kImuTopic
      << " — orientation.x/y/z/w, linear_acceleration.x/y/z\n";
  out << "  Plot/Raw: Odometry topic " << viewer::kImuOdomTopic
      << " (nav_msgs/Odometry; no TF frame — not in 3D)\n";
  out << "  Plot/Raw: Odometry topic " << viewer::kOdomTopic << " (nav_msgs/Odometry)\n";
}
...
void print_help() {
  ...
  help_cmd(out, color, "debug", "persons overlay and Foxglove bridge");
  help_cmd(out, color, "detect", "person detections source");
  ...
  out << '\n';
  help_header(out, color, "DEBUG");
  out << "  " << pad_right("debug on", 18);
  paint(out, color, kDim, "publish persons overlay and start Foxglove bridge");
  out << '\n';
  out << "  " << pad_right("debug off", 18);
  paint(out, color, kDim, "stop persons overlay and Foxglove bridge");
  out << '\n';
  out << "  " << pad_right("debug", 18);
  paint(out, color, kDim, "print overlay and bridge on/off");
  out << '\n';
  help_header(out, color, "DETECT");
  out << "  " << pad_right("detect offline", 18);
  paint(out, color, kDim, "use onboard YOLO11n");
  out << '\n';
  out << "  " << pad_right("detect mac", 18);
  paint(out, color, kDim, "use Mac detections");
  out << '\n';
  out << "  " << pad_right("detect", 18);
  paint(out, color, kDim, "print source mac/offline");
  out << '\n';
...
void print_debug(const units::DebugChange& change) {
  std::cout << "T1CTL_DEBUG_OK=" << (change.ok ? "1" : "0") << '\n';
  if (change.have_overlay) {
    std::cout << "overlay: " << (change.overlay_on ? "on" : "off") << '\n';
  }
  if (change.have_bridge) {
    std::cout << "bridge: " << (change.bridge_on ? "on" : "off") << '\n';
    if (change.bridge_on) {
      std::cout << "websocket: ws://" << change.websocket_host << ':' << viewer::kBridgePort
                << '\n';
    }
  }
}
...
void print_debug_error(const units::Status& status, const std::string& detail) {
  ...
  std::cerr.flush();
  if (status.demo == units::Demo::Inactive) {
    out << "\n  t1ctl start\n";
  }
}

void print_detect(const units::DetectChange& change) {
  std::cout << "T1CTL_DETECT_OK=" << (change.ok ? "1" : "0") << '\n';
  if (change.ok && change.have_source) {
    std::cout << "source: " << (change.source_offline ? "offline" : "mac") << '\n';
  }
}
...
void print_viewer_status(const viewer::Status& status) {
  const bool color = stdout_color();
  print_viewer_kv(std::cout, color, status);
  if (status.demo == viewer::DemoContour::Inactive) {
    std::cout << "\n  t1ctl start\n";
    return;
  }
  if (status.bridge == viewer::Bridge::Active && status.port_listening) {
    print_viewer_connect_hint(std::cout, color, status);
    return;
  }
  std::cout << "\n  t1ctl debug on\n";
}
...
void print_viewer_started(const viewer::Status& status) {
  const bool color = stdout_color();
  std::cout << "Viewer bridge started.\n\n";
  print_viewer_kv(std::cout, color, status);
  print_viewer_connect_hint(std::cout, color, status);
  std::cout << '\n';
}
...
void print_viewer_stopped(const viewer::Status& status) {
  const bool color = stdout_color();
  std::cout << "Viewer bridge stopped.\n\n";
  print_viewer_kv(std::cout, color, status);
}

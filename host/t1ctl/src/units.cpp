...
#include "ros_debug_embed.hpp"
#include "ros_detect_embed.hpp"
...
const char* debug_command_action(DebugCommand command) {
  switch (command) {
    case DebugCommand::On:
      return "on";
    case DebugCommand::Off:
      return "off";
    case DebugCommand::Status:
      return "status";
  }
  return "status";
}

std::string ros_debug_script(DebugCommand command) {
  return std::string(
             "export PYTHONUNBUFFERED=1; "
             "export T1CTL_DEBUG_ACTION=") +
         debug_command_action(command) +
         "; source /home/ubuntu/ros2_ws/.hiwonderrc >/dev/null 2>/dev/null; "
         "export ROS_LOCALHOST_ONLY=0; "
         "source /home/ubuntu/mentorpi_t1_ws/install/setup.bash >/dev/null 2>/dev/null; "
         "python3 -c " +
         shell_single_quote(kRosDebugPy);
}

std::vector<std::string> ros_debug_args(DebugCommand command) {
  return {"docker",       "exec",         "-u",        "ubuntu", "-w",
          "/home/ubuntu", kOursContainer, "/bin/bash", "-c",     ros_debug_script(command)};
}

const char* detect_command_action(DetectCommand command) {
  switch (command) {
    case DetectCommand::Offline:
      return "offline";
    case DetectCommand::Mac:
      return "mac";
    case DetectCommand::Status:
      return "status";
  }
  return "status";
}

std::string ros_detect_script(DetectCommand command) {
  return std::string(
             "export PYTHONUNBUFFERED=1; "
             "export T1CTL_DETECT_ACTION=") +
         detect_command_action(command) +
         "; source /home/ubuntu/ros2_ws/.hiwonderrc >/dev/null 2>/dev/null; "
         "export ROS_LOCALHOST_ONLY=0; "
         "source /home/ubuntu/mentorpi_t1_ws/install/setup.bash >/dev/null 2>/dev/null; "
         "python3 -c " +
         shell_single_quote(kRosDetectPy);
}

std::vector<std::string> ros_detect_args(DetectCommand command) {
  return {"docker",       "exec",         "-u",        "ubuntu", "-w",
          "/home/ubuntu", kOursContainer, "/bin/bash", "-c",     ros_detect_script(command)};
}

void fail_debug_change(DebugChange* change, const std::string& detail) {
  if (change == nullptr) {
    return;
  }
  change->ok = false;
...
void fail_detect_change(DetectChange* change, const std::string& detail) {
  if (change == nullptr) {
    return;
  }
  change->ok = false;
...
bool parse_debug_result(const std::string& text, DebugChange* change) {
  ...
  while (std::getline(ss, line)) {
    trim_ws(line);
    if (line == "T1CTL_DEBUG_OK=1") {
      parsed.ok = true;
      have_ok = true;
    } else if (line == "T1CTL_DEBUG_OK=0") {
      parsed.ok = false;
      have_ok = true;
    } else if (line.rfind("overlay:", 0) == 0) {
      std::string value = line.substr(8);
      trim_ws(value);
      if (value == "on") {
        parsed.have_overlay = true;
        parsed.overlay_on = true;
      } else if (value == "off") {
        parsed.have_overlay = true;
        parsed.overlay_on = false;
      }
    } else if (line.rfind("detail:", 0) == 0) {
      std::string value = line.substr(7);
      trim_ws(value);
  ...
  if (parsed.ok && !parsed.have_overlay) {
    return false;
  }
  *change = parsed;
  return true;
...
bool run_debug(DebugCommand command, DebugChange* change, Status* known) {
  return run_debug(posix_runner(), command, change, known);
}

namespace {

void fill_bridge_state(DebugChange* change, const viewer::Status& viewer_status) {
  if (change == nullptr) {
    return;
  }
  change->have_bridge = true;
  change->bridge_on =
      viewer_status.bridge == viewer::Bridge::Active && viewer_status.port_listening;
  change->websocket_host = viewer_status.host;
}
...
bool run_debug(ProcessRunner& runner, DebugCommand command, DebugChange* change, Status* known) {
  ...
  *status = Status{};
  ...
  if (!container_running(runner, kOursContainer)) {
    fail_debug_change(change, "container mentorpi-t1 is not running");
    return false;
  }

  std::string out;
  runner.run(ros_debug_args(command), &out);
  DebugChange overlay_part;
  bool overlay_ok = false;
  if (parse_debug_result(out, &overlay_part)) {
    overlay_ok = overlay_part.ok;
    if (overlay_part.have_overlay) {
      change->have_overlay = true;
      change->overlay_on = overlay_part.overlay_on;
    }
    if (!overlay_ok && !overlay_part.detail.empty()) {
      append_detail_line(&change->detail, overlay_part.detail);
    }
  } else if (!overlay_part.detail.empty()) {
    append_detail_line(&change->detail, overlay_part.detail);
  } else {
    append_detail_line(&change->detail, "debug helper failed");
  }

  bool bridge_ok = false;
  viewer::Status viewer_status;
  std::string bridge_detail;
  if (command == DebugCommand::On) {
    bridge_ok = viewer::start(runner, &viewer_status, &bridge_detail);
  } else if (command == DebugCommand::Off) {
    bridge_ok = viewer::stop(runner, &viewer_status);
  } else {
    viewer_status = viewer::query(runner);
    bridge_ok = true;
  }
  fill_bridge_state(change, viewer_status);
  if (!bridge_ok) {
    if (!bridge_detail.empty()) {
      append_detail_line(&change->detail, bridge_detail);
    } else if (command == DebugCommand::On) {
      append_detail_line(&change->detail, "viewer bridge start failed");
    } else if (command == DebugCommand::Off) {
      append_detail_line(&change->detail, "viewer bridge stop failed");
    }
  }

  change->ok = overlay_ok && bridge_ok;
  return change->ok;
}
...
bool parse_detect_result(const std::string& text, DetectChange* change) {
  ...
  while (std::getline(ss, line)) {
    trim_ws(line);
    if (line == "T1CTL_DETECT_OK=1") {
      parsed.ok = true;
      have_ok = true;
    } else if (line == "T1CTL_DETECT_OK=0") {
      parsed.ok = false;
      have_ok = true;
      ...
      if (value == "offline") {
        ...
        parsed.source_offline = true;
      } else if (value == "mac") {
        parsed.have_source = true;
        parsed.source_offline = false;
      }
    } else if (line.rfind("detail:", 0) == 0) {
      std::string value = line.substr(7);
      trim_ws(value);
  ...
  if (parsed.ok && !parsed.have_source) {
    return false;
  }
  *change = parsed;
  return true;
...
bool detect_source(DetectCommand command, DetectChange* change, Status* known) {
  return detect_source(posix_runner(), command, change, known);
}
...
bool detect_source(ProcessRunner& runner, DetectCommand command, DetectChange* change,
  ...
  *status = Status{};
  ...
  if (!container_running(runner, kOursContainer)) {
    fail_detect_change(change, "container mentorpi-t1 is not running");
    return false;
  }

  std::string out;
  runner.run(ros_detect_args(command), &out);
  DetectChange parsed;
  if (!parse_detect_result(out, &parsed) || !parsed.ok) {
    if (parsed.detail.empty()) {
      fail_detect_change(change, "detect helper failed");
    } else {
      fail_detect_change(change, parsed.detail);
    }
    return false;
  }
  *change = parsed;
